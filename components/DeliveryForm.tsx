...
const DeliveryForm: React.FC<DeliveryFormProps> = ({ onAddDelivery }) => {
  ...
  const [status, setStatus] = useState<DeliveryStatus>(DeliveryStatus.Completo);
  ...
  const isCompleto = status === DeliveryStatus.Completo;
  ...
  const validateForm = (): boolean => {
    ...
    if (!isCompleto) {
      if (!invoice.trim()) newErrors.invoice = "El número de factura es obligatorio.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Create a FormData object to send data. This is more reliable for Apps Script.
      const formData = new FormData();
      Object.entries(newDelivery).forEach(([key, value]) => {
          formData.append(key, String(value)); // All values must be strings for FormData
      });

      // Send data to Google Sheets using 'no-cors' mode with FormData.
      fetch(GOOGLE_SCRIPT_URL, {
        method: 'POST',
        body: formData,
        mode: 'no-cors',
      }).catch(err => {
        // This will only catch fundamental network errors, not HTTP status codes from the script.
      ...
      // Step 3: Update local state and save last used zonal
      ...
      // Step 5: Reset form but keep the zonal
      setSku('');
      setQuantity('');
      setInvoice('');
      setStatus(DeliveryStatus.Completo);
      setReceptionDate(new Date().toISOString().split('T')[0]);
      setErrors({});

    } catch (error) {
      console.error("Error during submission process (likely Gemini):", error);
      const errorMessage = error instanceof Error ? error.message : "Ocurrió un error inesperado al procesar la entrega.";
  ...
  const baseInputClasses = "w-full p-3 bg-slate-50 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 transition-shadow";
  const disabledInputClasses = "disabled:bg-slate-200 disabled:cursor-not-allowed disabled:text-slate-400";
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <label className={labelClasses}>Estado de la Entrega</label>
        <div className="grid grid-cols-3 gap-2 rounded-lg bg-slate-200 p-1">
          {Object.values(DeliveryStatus).map(s => (
            <button key={s} type="button" onClick={() => setStatus(s)} className={`py-2 px-4 rounded-md text-sm font-semibold transition-colors ${status === s ? 'bg-white text-sky-700 shadow' : 'bg-transparent text-slate-600 hover:bg-slate-300'}`}>
              {s}
            </button>
          ))}
        </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input type="text" id="sku" value={sku} onChange={e => setSku(e.target.value)} disabled={isCompleto} aria-disabled={isCompleto} className={inputClasses} placeholder={isCompleto ? 'N/A' : 'Ej: 12345-ABC'} />
          {errors.sku && <p className="text-red-500 text-xs mt-1">{errors.sku}</p>}
        </div>
        <div>
          <label htmlFor="quantity" className={labelClasses}>Diferencia Recibida</label>
          <input type="number" id="quantity" value={quantity} onChange={e => setQuantity(e.target.value)} disabled={isCompleto} aria-disabled={isCompleto} className={inputClasses} placeholder={isCompleto ? '0' : 'Ej: -5, 10'} />
      ...
      <div>
        ...
        <input type="text" id="invoice" value={invoice} onChange={e => setInvoice(e.target.value)} disabled={isCompleto} aria-disabled={isCompleto} className={inputClasses} placeholder={isCompleto ? 'N/A' : 'Ej: 900123456'} />
        {errors.invoice && <p className="text-red-500 text-xs mt-1">{errors.invoice}</p>}
      </div>

      <div>
        <button type="submit" disabled={isSubmitting} className="w-full flex justify-center items-center gap-3 bg-sky-600 text-white font-bold text-lg py-4 px-6 rounded-lg hover:bg-sky-700 disabled:bg-slate-400 transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">

...
const statusColorMap: Record<DeliveryStatus, string> = {
  [DeliveryStatus.Completo]: 'bg-green-100 text-green-800',
  [DeliveryStatus.Faltante]: 'bg-yellow-100 text-yellow-800',
  [DeliveryStatus.Sobrante]: 'bg-orange-100 text-orange-800',
};
...
const DeliveryTable: React.FC<DeliveryTableProps> = ({ deliveries }) => {
  return (
    <Card>
      <div className="p-6">
        <h2 className="text-2xl font-bold text-slate-700 mb-2">Historial de Recepciones</h2>
        <p className="text-slate-500 mb-6">Consulta aquí todos los registros de recepciones. La información es de solo lectura.</p>
      </div>
      ...
      {deliveries.length === 0 ? (
        <div className="text-center py-16 px-6">
          ...
          <h3 className="text-xl font-semibold text-slate-600">No hay registros aún</h3>
          <p className="text-slate-500 mt-2">Ve a la pestaña "Registrar" para añadir tu primera recepción.</p>
        </div>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm text-left text-slate-600">
            <thead className="bg-slate-100 text-xs text-slate-700 uppercase font-semibold">
              <tr>
            ...
            <tbody>
              {deliveries.map((delivery) => (
                <tr key={delivery.id} className="bg-white border-b hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-900">{delivery.zonal}</td>
                  <td className="px-6 py-4">{delivery.sku || 'N/A'}</td>
                  <td className="px-6 py-4">{delivery.invoice}</td>
